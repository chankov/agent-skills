...
3. **Use agents for review** — Copy `agents/code-reviewer.md` content when requesting structured code reviews. The full persona set lives in `agents/`: `code-reviewer`, `test-engineer`, `security-auditor`, `planner`, `plan-reviewer`, `builder`, `documenter`. Read-only personas (`code-reviewer`, `security-auditor`, `planner`, `plan-reviewer`) declare `tools: read,bash,grep,find,ls` and refuse to write.
